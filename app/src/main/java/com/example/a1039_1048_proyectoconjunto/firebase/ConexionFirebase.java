...
public class ConexionFirebase {
    ...
    public static boolean createDocument(String referencia, Object data, String idDocumento){
        for (Object x:getCollection(referencia)){
            Ubicacion ubicacion = (Ubicacion) x;
            if (ubicacion.equals((Ubicacion) data))
                return false;
        }
    ...
    public static boolean updateDocument(String referencia, Object data, String idDocumento){
        if (getDocument(referencia,idDocumento)==null){
            return false;
        }
        ...
        try {
            Response response = call.execute();
            if (response.isSuccessful() && getDocument(referencia, idDocumento).equals(data))
                return true;
        } catch (IOException e) {
            e.printStackTrace();
        }
